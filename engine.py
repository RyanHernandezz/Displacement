...
class Agent:
    def __init__(self, id, tier):
        ...
        if self.current_path == 'Emerging':
            self.base_reskill_speed = max(0.05, random.gauss(0.25, 0.05))
            self.info_quality = 1.2
            self.wealth = max(500, random.gauss(4000, 1000))
            self.age = max(18, min(65, random.gauss(26, 5)))
            ...
            self.age = max(18, min(65, random.gauss(40, 10)))
        elif self.current_path == 'Entrepreneur':
            self.base_reskill_speed = max(0.05, random.gauss(0.22, 0.05))
            self.info_quality = 1.1
            self.wealth = max(500, random.gauss(4500, 2000))
            self.age = max(18, min(65, random.gauss(35, 8)))
        else: # Credentialed
            self.base_reskill_speed = max(0.01, random.gauss(0.12, 0.03))
            self.info_quality = 0.8
            self.wealth = max(500, random.gauss(5000, 1500))
            self.age = max(18, min(65, random.gauss(48, 8)))
...
class Economy:
    def __init__(self, num_agents=400, exp_type='baseline', ablations=None):
        ...
        # Archetype Ablations (re-weight to ProbabilisticBayesian baseline)
        base_weights = {cls.__name__: weight for cls, weight in zip(arch_classes, arch_weights)}
        for cls_name in list(base_weights.keys()):
            # Keys from frontend will be like: ablation_arch_social, ablation_arch_narrative, etc.
            # We map class names to the frontend keys
            key_map = {
                'SocialMirror': 'ablation_arch_social',
                'NarrativeCaptive': 'ablation_arch_narrative',
                'StrategicOptions': 'ablation_arch_strategic',
                'Credentialist': 'ablation_arch_credentialist',
                'EmotionDriven': 'ablation_arch_emotion',
                'AnchoredExtrapolator': 'ablation_arch_anchored',
                'LossAversion': 'ablation_arch_loss'
            }
            if cls_name in key_map and self.ablations.get(key_map[cls_name], False):
                base_weights['ProbabilisticBayesian'] += base_weights[cls_name]
                base_weights[cls_name] = 0.0
                
        active_classes = [c for c in arch_classes if base_weights[c.__name__] > 0]
        active_weights = [base_weights[c.__name__] for c in active_classes]
        
        for i in range(num_agents):
            arch_type = random.choices(active_classes, weights=active_weights, k=1)[0]
        ...
        self.current_ai_wave = 0.0
        self.base_ai_shock = 0.0
    ...
    def _apply_macroeconomics(self):
        ...
        scale = len(self.agents) / 200.0
        for p in PATHS:
            base_d = BASE_DEMAND[p] * scale
    ...
    def trigger_experiment(self, exp_type):
        if exp_type == 'baseline':
            ...
            self.base_ai_shock = 0.95
            self.eco.ground_truth['Emerging'].val = 200
                
    def tick(self):
        self.tick_count += 1
        
        # Endogenous AI Shock and Reskilling Tech Growth
        self.ai_shock = max(self.base_ai_shock, min(1.0, self.tick_count * 0.00033)) # Gradually ramps up to 1.0 by tick 3000
        self.reskill_speed_multiplier = 1.0 + (self.tick_count * 0.0001) # Very slow linear growth
        ...
        if self.ai_shock > 0:
            # Create a non-perfectly periodic wave using interference and phase drift
            base_freq = self.tick_count * 0.05
            interference1 = math.sin(self.tick_count * 0.023)
            interference2 = math.cos(self.tick_count * 0.037)
            
            # Amplitude varies slowly with some jitter
            amp_mod = 1.0 + 0.4 * interference1 + random.uniform(-0.1, 0.1)
            
            # Phase is shifted by a slow wave, changing the apparent period
            drifted_phase = base_freq + interference2 * 1.5
            
            wave_val = math.sin(drifted_phase)
            wave = max(0, wave_val * amp_mod)
            acceleration = 1.0 + (self.tick_count * 0.0001)
            shock_magnitude = self.ai_shock * wave * acceleration * 0.1
            self.current_ai_wave = wave
            
            if shock_magnitude > 0 and not self.ablations.get('ablation_treadmill', False):
        ...
        for agent in updaters:
            if self.ablations.get('ablation_network', False):
                old_n = agent.neighbors
                agent.neighbors = []
                agent.process_signals(self.eco, self.agents)
                agent.neighbors = old_n
            else:
                agent.process_signals(self.eco, self.agents)
        ...
        for agent in decision_makers:
            ...
            if agent.state == 'Unemployed' and not self.ablations.get('ablation_necessity_ent', False):
                boost = 100
            ...
            if agent.desired_path != agent.current_path:
                ...
                if agent.skill_vectors[agent.desired_path] >= 1.5:
                    ...
                    if agent.state != 'Unemployed' and not self.ablations.get('ablation_financial', False):
                        agent.wealth -= 50 # Transition fee for employed
                    agent.is_reskilling = False
            elif agent.is_reskilling and agent.desired_path == agent.current_path:
                if agent.skill_vectors[agent.current_path] >= 1.5:
                    agent.is_reskilling = False
        ...
        for agent in self.agents:
            ...
            if agent.state != 'Unemployed':
                ...
                layoff_risk = 0.0002 if not self.ablations.get('ablation_frictional', False) else 0.0
                if tightness < 1.0:
            ...
            if agent.is_reskilling:
                ...
                if self.ablations.get('ablation_financial', False):
                    financial_runway = 1.0
                else:
                ...
                # 8. Age Penalty (Cemented identity & slower neuroplasticity)
                age_penalty = max(0.6, 1.0 - ((agent.age - 25) * 0.01))
                
                # Final Stochastic Growth
                final_speed = baseline_learning * adjacency * financial_runway * info_quality * employer_demand * time_availability * psych_willingness * age_penalty * self.reskill_speed_multiplier
                agent.last_reskill_speed = final_speed
                agent.skill_vectors[agent.desired_path] += max(0, random.gauss(final_speed, final_speed * 0.2))
            else:
                agent.last_reskill_speed = 0.0
            
            # Wealth Mechanics (Now that wage is finalized)
            living_cost = 80
            if agent.state != 'Unemployed':
                agent.wealth += agent.wage - living_cost
            else:
                # Capital drain (costs money to reskill)
                if agent.is_reskilling and not self.ablations.get('ablation_financial', False):
                    agent.wealth -= 30 
            ...
            if agent.state == 'Unemployed':
                agent.wage = 0
                if not self.ablations.get('ablation_financial', False):
                    agent.wealth -= 30 # Drain savings while unemployed
                agent.consecutive_unemployed_ticks += 1
                agent.total_unemployed_ticks += 1
                
                displacement_rate = self.ai_shock * 0.1
                effective_reskill_rate = getattr(agent, 'last_reskill_speed', 0.0)
                
                # Structural Disruption: Stuck in unemployment and fundamentally unable to reskill fast enough to escape
                if agent.consecutive_unemployed_ticks > 250 and effective_reskill_rate < displacement_rate:
            ...
            if agent.wealth < 0 and not self.ablations.get('ablation_financial', False):
                agent.state = 'Unemployed'
                agent.is_reskilling = False
        ...
        path_counts = {p: sum(1 for a in self.agents if a.current_path == p and a.state != 'Unemployed') for p in PATHS}
    ...
    def generate_longitudinal_report(self):
        ...
        for a in self.agents:
            ...
            if key not in groups:
                groups[key] = {'count': 0, 'unemployed_ticks': 0, 'structural_unemp': 0, 'emerging': 0, 'age_sum': 0}
            
            groups[key]['count'] += 1
            groups[key]['unemployed_ticks'] += a.total_unemployed_ticks
            groups[key]['age_sum'] += getattr(a, 'age', 35)
